...
class RealColibriV1(object):
    ...
    def load_dataset(self):
        ...
        if os.path.exists(cache_path) and self.use_cache:
            ...
            print(f"Cached information for dataset {self.name} loaded from {cache_path}")
        else:
            self.samples = []
            # Load meta data
            print(f"meta_root: {self.meta_root}")
            ...
            if os.path.exists(split_file):
                ...
                for sample_name in sample_names: #glob(os.path.join(self.meta_root, "*.pkl")):
                    parsed = re.search("rec([0-9]+)_([0-9]+)", sample_name)
                    sample_file = os.path.join(self.meta_root, f"{sample_name}.pkl")
                    sample = pickle.load(open(sample_file, "rb"))
                    sample['model_name'] = "drill_segmentation_textured_final.ply"
                    sample['grasp_no'] = parsed.group(1)
                    sample['frame_no'] = parsed.group(2)
    ...
    def get_image(self, idx):
        ...
        img_path = os.path.join(self.rgb_root, f"rec{sample['grasp_no']}_{sample['frame_no']}.jpg")
        img = Image.open(img_path).convert("RGB")
        return img

    def get_obj_mask(self, idx):
        sample = self.get_dataidx(idx)
        img_path = os.path.join(self.segm_root, "rec{}_{}.png".format(sample['grasp_no'], sample['frame_no']))
        img = Image.open(img_path).convert("RGB")
    ...
    def get_joints3d(self, idx):
        sample = self.get_dataidx(idx)
        joints3d = sample['coords_3d'] # shape: (n, 3)
        trans_joints3d = utils.transform(joints3d, self.get_camextr(idx), convert_to_homogeneous=True)
        return np.array(trans_joints3d).astype(np.float32)

    def get_joints2d(self, idx):
        joints3d = self.get_joints3d(idx)  # shape: (nJoints,3)
        assert joints3d.shape[1] == 3, "get_joints3d returned wrong shape: {}".format(joints3d.shape)
        hom_joints2d = np.dot(self.get_camintr(idx), joints3d.transpose())  # shape: (3,nJoints)
        # Normalize homogeneous coordinates
        hom_joints2d = hom_joints2d / hom_joints2d[2, :]
        joints2d = hom_joints2d[:2, :].transpose()  # shape: (nJoints,2)
        return np.array(joints2d).astype(np.float32)
